...
namespace CyberPulse.Systems
{
    ...
    public class SongAnalyzer : MonoBehaviour
    {
        ...
        private void Start()
        {
            if (_analyzeOnStart && _musicSource != null && _musicSource.clip != null)
        ...
        private float DetectBPM(List<float> onsetTimes)
        {
            ...
            foreach (var kvp in histogram)
            {
                if (kvp.Key >= 80 && kvp.Key <= 160 && kvp.Value > midVotes)
                {
                    midVotes  = kvp.Value;
                    midBucket = kvp.Key;
                }
            }
            if (midBucket >= 0 && midVotes >= bestVotes - 2)
                bestBucket = midBucket;
