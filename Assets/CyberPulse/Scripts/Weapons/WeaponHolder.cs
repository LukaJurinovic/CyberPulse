...
namespace CyberPulse.Weapons
{
    ...
    public class WeaponHolder : MonoBehaviour
    {
        ...
        private void Start()
        {
            if (SyncGauge.Instance != null)
                SyncGauge.Instance.OnSyncFull += OnSyncFull;
        }
        ...
        private void OnEnable()
        {
            ...
            _input.WeaponSlot1Input   += OnSlot1;
            _input.WeaponSlot2Input   += OnSlot2;
            _input.WeaponSlot3Input   += OnSlot3;
            _input.AltFirePressed     += OnAltFirePressed;
            _input.AltFireReleased    += OnAltFireReleased;
        }
        ...
        private void OnSlot1() => SwitchTo(0);
        private void OnSlot2() => SwitchTo(1);
        private void OnSlot3() => SwitchTo(2);
        ...
        {
            TimeManager.Instance?.EnterSlowMo();
            if (ActiveWeapon != null)
                SyncGauge.Instance?.TrySpend(ActiveWeapon.SpecialCost, ActiveWeapon);
        }

        private void OnAltFireReleased() => TimeManager.Instance?.ExitSlowMo();

        private void OnSyncFull()
        {
            SyncGauge.Instance?.Subtract(100f);
